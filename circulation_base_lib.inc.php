...
class circulation extends member
{
    ...
    public function returnItem($int_loan_id)
    {
        ...
        if (isset($_SESSION['receipt_record'])) {
            // get item data
            $_title_q = $this->obj_db->query('SELECT b.title, l.item_code, l.loan_date FROM loan AS l
                LEFT JOIN item AS i ON l.item_code=i.item_code
                INNER JOIN biblio AS b ON i.biblio_id=b.biblio_id WHERE l.loan_id='.$int_loan_id);
            $_title_d = $_title_q->fetch_assoc();
            $_SESSION['receipt_record']['return'][$_title_d['item_code']] = array('itemCode' => $_title_d['item_code'], 'title' => $_title_d['title'], 'loanDate' => $_title_d['loan_date'], 'returnDate' => $_return_date, 'overdues' => $_fines);
        }
        // check if this item is being reserved by other member
    ...
    public function extendItemLoan($int_loan_id)
    {
        ...
        if (isset($_SESSION['receipt_record'])) {
            ...
            $_title_q = $this->obj_db->query('SELECT b.title, l.item_code, l.loan_date FROM loan AS l
                ...
                INNER JOIN biblio AS b ON i.biblio_id=b.biblio_id WHERE l.loan_id='.$int_loan_id);
            $_title_d = $_title_q->fetch_assoc();
            $_SESSION['receipt_record']['extend'][$_title_d['item_code']] = array('itemCode' => $_title_d['item_code'], 'title' => $_title_d['title'], 'loanDate' => $_title_d['loan_date'], 'extendDate' => $_loan_date, 'dueDate' => $_due_date);
        }
        return true;
    }
            ...
            foreach ($_SESSION['temp_loan'] as $loan_item) {
                ...
                if (!$sql_op->insert('loan', $data)) {
                    ...
                    if (isset($_SESSION['receipt_record'])) {
                        ...
                        $_title = $_title_d[0];
                        // add to receipt
                        $_SESSION['receipt_record']['loan'][$data['item_code']] = array('itemCode' => $data['item_code'], 'title' => $_title, 'loanDate' => $data['loan_date'], 'dueDate' => $data['due_date']);
                    }
